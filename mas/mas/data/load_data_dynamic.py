...
def load_SRTR_dynamic(
    ...
    dataset only for multiple imputation datasets
    """
    if load_from_pkl:

        if imputation == "mice" or imputation == "mice_gbdt":
            train = pd.read_pickle(f"mas/data/dynamic_postprocessed/{imputation}/train_dynamic_{dataset}.pkl")
            val = pd.read_pickle(f"mas/data/dynamic_postprocessed/{imputation}/val_dynamic_{dataset}.pkl")
            test = pd.read_pickle(f"mas/data/dynamic_postprocessed/{imputation}/test_dynamic_{dataset}.pkl")
        else:
            train = pd.read_pickle(f"mas/data/dynamic_postprocessed/{imputation}/train_dynamic.pkl")
            val = pd.read_pickle(f"mas/data/dynamic_postprocessed/{imputation}/val_dynamic.pkl")
            test = pd.read_pickle(f"mas/data/dynamic_postprocessed/{imputation}/test_dynamic.pkl")
        ...
        # fol_immuno = pd.read_sas(DATA_DIR + "fol_immuno.sas7bdat")
        ...
        for split in ["train", "val", "test"]:
            with open(f"mas/data/data_splits/{split}_split.txt") as f:
        ...
        elif imputation == "mean" or imputation == "median":
            train = txf_li[txf_li['PX_ID'].isin(px_ids["train"])]
            val = txf_li[txf_li['PX_ID'].isin(px_ids["val"])]
            test = txf_li[txf_li['PX_ID'].isin(px_ids["test"])]

            train, means = eval(f"processor.{imputation}(train)")
            val, _ = eval(f"processor.{imputation}(val, means)")
            test, _ = eval(f"processor.{imputation}(test, means)")
        else:
            train = txf_li[txf_li['PX_ID'].isin(px_ids["train"])]
            val = txf_li[txf_li['PX_ID'].isin(px_ids["val"])]
            test = txf_li[txf_li['PX_ID'].isin(px_ids["test"])]

            train, kernel = eval(f"processor.{imputation}(txf_li)")
            val_ids, test_ids = val["PX_ID"], test["PX_ID"]

            val = kernel.impute_new_data(new_data=val.drop(columns=["PX_ID"]), random_state=42)
            val_completed = []
            for i in range(5):
                completed_data = kernel.complete_data(dataset=i)
                completed_data["PX_ID"] = val_ids
                val_completed.append(completed_data)

            test = kernel.impute_new_data(new_data=test.drop(columns=["PX_ID"]), random_state=42)
            test_completed = []
            for i in range(5):
                completed_data = kernel.complete_data(dataset=i)
                completed_data["PX_ID"] = test_ids
                test_completed.append(completed_data)

            for i in range(5):
                train[i].to_pickle(f"mas/data/dynamic_postprocessed/{imputation}/train_dynamic_{i}.pkl")
                val_completed[i].to_pickle(f"mas/data/dynamic_postprocessed/{imputation}/val_dynamic_{i}.pkl")
                test_completed[i].to_pickle(f"mas/data/dynamic_postprocessed/{imputation}/test_dynamic_{i}.pkl")

            return

        train.to_pickle(f"mas/data/dynamic_postprocessed/{imputation}/train_dynamic.pkl")
        val.to_pickle(f"mas/data/dynamic_postprocessed/{imputation}/val_dynamic.pkl")
        test.to_pickle(f"mas/data/dynamic_postprocessed/{imputation}/test_dynamic.pkl")
...
class ImputeProcessor():
    ...
    def mean(self, df, means = None):
        ids, time = df["PX_ID"], df["TIME_SINCE_BASELINE"]

        df = df.drop(["PX_ID", "TIME_SINCE_BASELINE"], axis=1)
        if means is None:
            means = df.mean(axis=0)
        df = df.fillna(means)
        
        df['PX_ID'] = ids; df['TIME_SINCE_BASELINE'] = time
        return df, means
    
    def median(self, df, medians = None):
        ids, time = df["PX_ID"], df["TIME_SINCE_BASELINE"]
    ...
    def mice_gbdt(self, df):
        ...
        kernel.mice(iterations=2, boosting='gbdt', verbose=True, variable_parameters=optimal_parameters)
        completed = []
        for i in range(5):
            completed_data = kernel.complete_data(dataset=i)
            completed_data["PX_ID"] = ids
...
if __name__ == "__main__":
    ...
    # load_SRTR_dynamic(imputation="mean", load_from_pkl=False)
    # load_SRTR_dynamic(imputation="median", load_from_pkl=False)
