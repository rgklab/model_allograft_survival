...
Hu, Shi, et al. "Transformer-based deep survival analysis." Survival Prediction-Algorithms, Challenges and Applications. PMLR, 2021.
Thanks to Shi Hu for sharing the code.
"""
...
class MASFormerModel(pl.LightningModule):
    def __init__(self, hparams):
        super().__init__()
        self.hp = hparams
        self.save_hyperparameters()
        self.model = MASFormer(self.hp["num_features"], self.hp["num_layers"], 
                self.hp["d_model"], self.hp["d_ff"], self.hp["num_heads"], self.hp["drop_prob"])
    ...
    def training_step(self, batch, batch_idx):
        ...
        sigmoid_a = self.forward(features[0], mask_a) * mask_a
        surv_probs_a = torch.cumprod(sigmoid_a, dim=1)
        loss = nn.BCELoss()(surv_probs_a * mask_a, label_a * mask_a)

        sigmoid_b = self.forward(features[1], mask_b) * mask_b
    ...
    def validation_step(self, batch, batch_idx):
        ...
        new_sigmoid = self.forward(features[0], mask_a) * new_mask
        harzard = 1 - new_sigmoid
...
def run(out_dir: str, feature: str="full"):
    ...
    hparams = {"num_heads":4, "d_model":512, "drop_prob":0.1, "num_layers":4, "d_ff":2048,
            "learning_rate":1e-4, "coeff":0.1, "num_features":287, "feature":feature, "batch_size":16}
    if feature == "mas":
...
def debug(feature:str="full"):
    ...
    seed_everything(42, workers=True)
    hparams = {"num_heads":4, "d_model":512, "drop_prob":0.1, "num_layers":4, "d_ff":2048,
            "learning_rate":1e-4, "coeff":0.1, "num_features":263, "feature":feature, "batch_size":16}
