...
def load_SRTR_static_df(
    ...
    if load_from_pkl:
        train = pd.read_pickle(f"masformer/data/static_postprocessed/train_static_{outcome}_{feature}.pkl")
        val = pd.read_pickle(f"masformer/data/static_postprocessed/val_static_{outcome}_{feature}.pkl")
        test = pd.read_pickle(f"masformer/data/static_postprocessed/test_static_{outcome}_{feature}.pkl")
        ...
        train_df_static.to_pickle(f"masformer/data/static_postprocessed/train_static_{outcome}_{feature}.pkl")
        val_df_static.to_pickle(f"masformer/data/static_postprocessed/val_static_{outcome}_{feature}.pkl")
        test_df_static.to_pickle(f"masformer/data/static_postprocessed/test_static_{outcome}_{feature}.pkl")
...
def load_SRTR_static(
    ...
    for split in ["train", "val", "test"]:
        px_ids[split] = np.loadtxt(f"masformer/data/data_splits/{split}_split.txt", delimiter='\n')
    ...
    def extract_graft_XTE(self, tx_lis, txf_li, n_years):
        ...
        for set, tx_li in zip(sets, tx_lis):
            ...
            for i, id in tqdm(enumerate(tx_li.PX_ID), desc='Processing Graft Faliure Outcomes', total=len(tx_li)):
                row_txf_li = txf_li[txf_li.PX_ID == id].copy()
                if row_txf_li.shape[0] == 0:
                    # no follow-up, censored at transplant time
                    pass
                else:
